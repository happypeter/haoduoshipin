...
需要注意的是，我们没有一次导入所有的模块供整个应用使用，而是当一个模块依赖于另一个模块的时候 —— 比如上述代码需要一个 lodash 方法 —— 我们才导入这个方法到需要它的模块。如果有5个模块需要同样的 lodash 函数，我们就导入它5次。这有利于维持一个清晰的范围也使得模块具有非常好的可移植性和可重用性。
...
有几个流行的打包工具，其中多数依赖于 Babel 把 ES6 模块转译为 CommmonJS 模块。
